...
1. **Find the right test file** — mirror the source path. `lib/decidulixir/graph/node.ex` → `test/decidulixir/graph/node_test.exs`. Read the existing test file to match conventions (aliases, struct construction, assertion style).
