...
1. **Find the right test file** — follow the project's mirrored test structure. If the root cause is in `lib/decidulixir/graph/node.ex`, the test goes in `test/decidulixir/graph/node_test.exs`. If the file doesn't exist, create it.
...
4. **Run the test and confirm it fails:**
   ```bash
   mix test test/decidulixir/graph/node_test.exs --only line:<line_number>
   ```
